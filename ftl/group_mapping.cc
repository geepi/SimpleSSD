...
bool GroupMapping::initialize() {
  ...
  debugprint(LOG_FTL_GROUP_MAPPING, "Initialization started");
  ...
  debugprint(LOG_FTL_GROUP_MAPPING, "Total logical pages: %" PRIu64,
             nTotalLogicalPages);
  debugprint(LOG_FTL_GROUP_MAPPING,
  ...
  debugprint(LOG_FTL_GROUP_MAPPING, "Filling finished. Page status:");
  debugprint(LOG_FTL_GROUP_MAPPING,
  ...
  debugprint(LOG_FTL_GROUP_MAPPING, "Initialization finished");

  return true;
}

void GroupMapping::read(Request &req, uint64_t &tick) {
  uint64_t begin = tick;
...
void GroupMapping::trim(Request &req, uint64_t &tick) {
  uint64_t begin = tick;
...
void GroupMapping::format(LPNRange &range, uint64_t &tick) {
...
Status *GroupMapping::getStatus(uint64_t lpnBegin, uint64_t lpnEnd) {
...
float GroupMapping::freeBlockRatio() {
  return (float)nFreeBlocks / param.totalPhysicalBlocks;
}

uint32_t GroupMapping::convertBlockIdx(uint32_t blockIdx) {
  return blockIdx % param.pageCountToMaxPerf;
}

uint32_t GroupMapping::getFreeBlock(uint32_t idx) {
...
void GroupMapping::calculateVictimWeight(
...
void GroupMapping::selectVictimBlock(std::vector<uint32_t> &list,
                                     uint64_t &tick) {
...
void GroupMapping::doGarbageCollection(std::vector<uint32_t> &blocksToReclaim,
                                       uint64_t &tick) {
  PAL::Request req(param.ioUnitInPage);
...
void GroupMapping::readInternal(Request &req, uint64_t &tick) {
...
void GroupMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  PAL::Request palRequest(req);
...
void GroupMapping::trimInternal(Request &req, uint64_t &tick) {
  auto mappingList = table.find(req.lpn);
...
void GroupMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
...
float GroupMapping::calculateWearLeveling() {
...
void GroupMapping::calculateTotalPages(uint64_t &valid, uint64_t &invalid) {
  valid = 0;
  invalid = 0;
...
void GroupMapping::getStatList(std::vector<Stats> &list, std::string prefix) {
  Stats temp;
...
void GroupMapping::getStatValues(std::vector<double> &values) {
...
void GroupMapping::resetStatValues() {
  memset(&stat, 0, sizeof(stat));
}
